...
func _wait_and_register() -> void:
	...
	net.send_character_id.rpc_id(1, int(GameState.current_character["id"]))
...
func _logout_then_quit() -> void:
	...
	# D'abord on coupe la connexion réseau proprement
	var net := get_node_or_null("/root/NetworkClient")
	if net != null:
		net.connected = false
	...
	# Ensuite on met offline via HTTP puis on quitte
